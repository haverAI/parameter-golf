...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 7))  # Physical layers (was 9)
    num_recurrence = int(os.environ.get("NUM_RECURRENCE", 2))  # Loop count → 7×2=14 effective
    num_kv_heads = int(os.environ.get("NUM_KV_HEADS", 4))
    model_dim = int(os.environ.get("MODEL_DIM", 512))  # Same as baseline
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Depth recurrence: loop through all physical layers num_recurrence times
        for _loop in range(self.num_recurrence):
            skips: list[Tensor] = []
